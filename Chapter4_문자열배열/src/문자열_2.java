...
public class 문자열_2 {
	...
	public static void main(String[] args) {
		...
		String[] title={"Kitsch","CHRISTIAN","꽃","Ditto",
				"Hype boy","Teddy Bear","사건의 지평선","OMG",
				"심 (心)","I Don't Think That I Like Her",
				"사랑은 늘 도망가","건물 사이에 피어난 장미","우리들의 블루스",
				"빙글빙글 (Prod. by R.Tee)","가질 수 없는 너","After LIKE",
				"다시 만날 수 있을까","밤이 무서워요 (Lonely Night)",
				"이제 나만 믿어요","London Boy","파이팅 해야지 (Feat. 이영지)",
				"Polaroid","무지개","Attention","아버지","A bientot",
				"LOVE DIVE","손이 참 곱던 그대","사랑인가 봐","사랑해 진짜",
				"연애편지","보금자리","인생찬가","ANTIFRAGILE","NIGHT DANCER",
				"Dangerously","Zero","The Drum","Like Crazy","바보가슴",
				"Nxde","사랑하기 싫어","새삥 (Prod. by ZICO) (Feat. 호미들)",
				"That's Hilarious","Nostalgia","흔들리는 꽃들 속에서 네 샴푸향이 느껴진거야",
				"Monologue","VIBE (Feat. Jimin of BTS)","Dynamite","오르트구름",
				"그때 그 순간 그대로 (그그그)","다정히 내 이름을 부르면","TOMBOY",
				"너의 모든 순간","해요 (2022)","Pink Venom","Unholy","Shut Down","취중고백",
		...
		// 1. startsWith : 시작문자열 같을 때 가져오기
		/* Scanner scan=new Scanner(System.in);
		System.out.print("검색어 입력:");
		String fd=scan.next(); // 문자열 받을 때 사용 // 문자열: next(), 정수: nextInt(), 실수: newxtDouble(), 논리: nextBoolean()
		for(String s:title) {
			if(s.startsWith(fd)) {
				System.out.println(s);
//				try {
//					Thread.sleep(500);
//				}catch(Exception ex) {} // 하나씩 천천히 나타나게 하는 효과
			}
		}
		// 2. endsWith : 끝문자열 같을 때 가져오기
		...
		// 3. contains : 포함된 단어 가져오기
		System.out.println("===== contains(단어포함) =====");
		for(String s:title) {
		...
		while(true)
		{
			...
			if(menu==1) {
				for(int i=0; i<title.length; i++) { // 배열 여러 개를 동시에 출력할 땐 일반 for문
			...
			if(menu==2) {
				System.out.print("검색어 입력:");
				String fd=scan.next();
				for(String s:title) { // 한 곳에서 찾아올 때 for-each
					if(s.contains(fd)) {
						System.out.println(s);
					}
				}
			...
			if(menu==3) {
				...
				String fd=scan.next();
				for(int i=0;i<singer.length;i++) { // 배열이 다른 곳에서 찾아올 때 일반 for문
