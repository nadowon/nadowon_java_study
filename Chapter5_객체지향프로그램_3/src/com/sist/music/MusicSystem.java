...
public class MusicSystem {
	Music[] music=new Music[50]; // null, 배열이 생성됐다고 해서 메모리 할당된 게 아님 // 배열 50개를 만들어라 // 주소 없는 상태
	// List<Music>
	// 1. 메소드 => 생성자
	void init()
	{
		...
		for(int i=0;i<MusicData.title.length;i++) {
			...
			music[i]=new Music(); // Music 크기의 메모리 공간을 50개 생성해라 // 메모리 할당
	...
	/*public static void main(String[] args) {
		MusicSystem ms=new MusicSystem(); // 인스턴스 메소드는 메모리 할당을 먼저하고
		ms.init();                        // 구현
		Scanner scan=new Scanner(System.in);
