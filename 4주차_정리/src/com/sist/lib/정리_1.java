 ...
 *               컴파일 시에 예외처리 여부 확인      예외처리 여부를 미확인
 *               => 예외처리 반드시 한다           => 필요시에만 작업
 ...
 *                    -----------------------------------------
 *                    |                                       |
 *               IOException : file명/디렉토리명             실행 시 에러 (인터프리터)
 *               ClassNotFoundException                 RuntimeException
 *               => Class.forName("패키지명.클래스명");           |
 *               => 웹(MVC)                              NullPointerException : 초기화가 안 된 상태 
 *               InterruptedException (쓰레드)               String s; => 선언만 하고 값을 안 줌
 *               MalformedURLException (ip,web:네트워크)       s.replace()
 *               SQLException (문법 => 오라클)             NumberFomatException
 *                                                         Integer.parseInt(" 10")
 *                                                         web
 *                                                         ---
 *                                                         데이터 전송
 ...
 *            
 *            ***= 예외회피:예외 떠넘기기 (라이브러리)
 *                 사용법) 메소드 선언 뒤에 예상되는 에러를 선언 => 메소드 호출 시에 처리할 수 있게 만든다
 *                        => 사용하는 프로그래머가 처리해서 사용
 *                        1) 선언
 *                        2) try~catch
 ...
 *                     
 *                 선언된 예외클래스가 많은 경우
 *                 1) 전체를 사용 catch()를 여러 개 사용
 *                 2) 전체를 포함 클래스로 통합
 *                    => Exception
 *                    => Throwable
 *                   예) public void method() throws IOException,InterruptedException,SQLException
 *                      {
 *                      }
 *                      
 *                      ==> 호출
 ...
 *            = 사용자 정의 예외처리 : 사용빈도 거의 없다
 *              Class MyException extends Exception
 *              => if문
 *   =================================================================================== 자바의 문법(8장)
 *    => 프로그램 상에서 제어가 안되는 경우가 많다
 *       예) 다른 프로그램과 연동(브라우저, 오라클...)
 *           파일 읽기 / 쓰기
 ...
 *             => import 생략
 *   라이브러리 => 변수는 없음, 메소드 기능 알아야함
 *   
 *   = Object
 *        ***Object clone() : 복제 (Spring에서는 prototype)
 *                         새로운 메모리 생성
 *        equals() : 객체 비교 (오버라이딩을 하기 전까지는 메모리 주소를 비교)
 *                            -------- 값을 비교 : String
 *        ***toString() : 객체를 문자열화 (default는 메모리 주소 리턴)
 ...
 *      => 검색기(Search Bar)
	...
	***3. 두개의 String에 값만을 비교해서 같으면 true, 다르면 false를 반환한다.(대소비교)
	    ...
	    => 로그인 처리, 아이디 중복... 상세보기
	***4. 지정한 문자가 문자열에 몇번째에 있는지를 반환한다. => String은 char[] => 인덱스번호 0부터 시작
	...
	***6. 문자열의 길이를 반환한다. => 문자의 갯수
	    int length()
	7. 정규표현식을 지정한 문자로 바꿔서 출력한다. => 통계/분석 => 추천 
	...
	***10. 문자열에 지정한 범위에 속하는 문자열을 반환한다.(시작범위에 값은 포함하고, 끝나는 범위에 값은 포함하지않는다.)
	...
	12. 문자열에 소문자를 대문자로 변환한다.
	    ...
	    => 오라클은 대소문자를 구분하지 않는다 (대소문자 구분은 실제 저장된 값)
	***13. 문자열을 그대로 반환해준다.
	    String toString()
	14. 문자열에 공백을 제거해 준다. => 좌우에 있는 공백만 제거
	    String trim()
	***15. 지정한 개체의 원시 값을 반환 (데이터형을 문자열로 변경) => 주소값
 ...
 *   
 *   = Wrapper : 기본데이터형을 사용하기 쉽게 기능을 부여 => 클래스
 *       int     ==> Integer
 *       double  ==> Double
 *       boolean ==> Boolean
 *       long    ==> Long
 ...
 *      ** 월(month) => 0부터 시작한다 ==> 등록 시에 month-1
 *         요일(week) => 1부터 시작한다 ==> 사용 시에 week-1
 *   = 문자열 분해
 *      StringTokenizer
 *       => counterTokens() : 분해된 문자열의 갯수
 *       => hasMoreTokens() : 데이터가 존재할 때까지 => true
 *          while을 이용해서 주로 사용
 *       => nextTokens() : 실제 분해된 데이터를 읽을 경우에 주로 사용
 *       ==> 오라클 연동 / 네트워크로 상대방에게 값을 전송
 *   = 자료구조 : 메모리에서 데이터를 쉽게 관리할 수 있게 만든 클래스
 *      Collection(데이터를 모아서 관리) => 가변형(크기를 결정하지 않는다)
 *      관리 프로그램을 제작(사람, 영화...)
 *      표준화(모든 프로그래머가 동일하게 사용) 예) 인터페이스
 *      
 *                     Collection
 *                          |
 *               -----------------------
 ...
 *                       |             ----------- 영구적인 저장장치
 *                 변수, 배열, 클래스 => 메모리 => 파일(임시저장) => (보안취약) => 오라클 서버
 *                                                                       구분(제약조건)
 *                                                                       => Key
 *              => ArrayList
 *                 많이 사용되는 곳 : WEB => 데이터를 모아서 전송할 때 (목록)
 *                 주로 사용 기능 (메소드)
 *                 ***1) 데이터 추가 : add()
 *                 2) 데이터 수정 : set()
 ...
 *             ***5) 전체 삭제 : clear()
 *             => HashSet
 *                많이 사용되는 곳 : 네트워크에서 서버에 접속하는 클라이언트의 IP,PORT / 웹 채팅
 *       Map : 순서가 없다, key-value => 두 개를 동시에 저장
 *             => HashMap
 *                많이 사용되는 곳 : WEB(지원하는 클래스 => Map)
 *                                클래스를 여러 개 모아서 관리(Spring, MyBatis)
 *                put() : 저장 시에 사용
 *                get(key) : 저장된 데이터 읽기
 *    
 *       1) 기본 생성자를 사용하게 되면 모든 데이터형은 Object
 *          List list=new ArrayList(); => 인터페이스여서 구현된 클래스로 메모리할당해야 함
 *          => 저장
 *             add(Object)
 ...
 *           => 데이터형이 섞여있으면 관리가 어려움 => for문 수행이 어렵다
 *           => 같은 데이터형을 첨부하는 것이 편리하다
 *           => Object 리턴이면 반드시 형변환을 시킨다
 *           => 메소드에서 사용되는 매개변수, 리턴형을 통일화 => 제네릭
 *              형변환할 필요가 없기 때문에 소스가 간결하다
 *              표준화시킬 수 있다
