...
public class BoardManager implements BoardInterface {
	 ...
	 *             예) 회원가입
	 * 		Map : 순서가 없다, 두 개를 동시에 저장
	 * 						key,value ==> key는 중복이 없다 / value는 중복이 가능
	 *             예) 데이터 송신/수신 => 웹에서 지원하는 대부분의 클래스가 Map형식으로 되어있다
	 */
	...
	static
	{
		 ...
		 *       }
		 *       
		 *     초기화 순서 : 명시적 초기화 - static{} - {} - 생성자
		 *     명시적 초기화는 선언과 동시에 값을 부여
		 *     구현이 필요할 경우에는 명시적 초기화를 사용할 수 없다
		 *     --------------
		 *     파일읽기, 데이터읽기, 메소드 호출 => 클래스 영역에서는 구현이 불가능하다
		 *       
		 */
