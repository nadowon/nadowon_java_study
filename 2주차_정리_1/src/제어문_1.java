 ...
 *    ------------- 댓글, 수정, 삭제...
 *    형식)
 *    	if(조건문) => 조건문 안에는 부정연산자, 비교연산자, 논리연산자만 들어갈 수 있다
 *      {
 *      	출력내용 => true일 때만 수행하는 문장
 *      }
 *      예) 번호가 3번인 내용을 출력해라 => 상세
 *      --------------------------------
 *      단일: 속도가 늦다 (독립적인 문장) => 중복적으로 수행해야할 때 사용
 *      ----------------------------------------------------
 *      다중조건
 *    
 *    	true/false를 나눠서 처리
 ...
 *    	if(id.equals("") && pws.equals("")) // 아이디가 틀렸는지 비번이 틀렸는지 모름
 *    
 *    	웹 => 일반 사용자 => 최대한 서비스 제공
 *    	중첩조건문
 *    	if(id.equals(""))
 ...
 *  ===============================================================  
 *    선택문 : 네트워크, 게임(키값)
 *    switch(정수,문자,문자열) => 해당 위치부터 break가 나올 때까지 수행
 *    				 ----
 *    {
 *    	case "Find":
 *    	case "Search";
 *    		처리
 *    		break; // Find와 Search 둘다 true될 경우 break
 *    	case "Login";
 *    		처리
 *    		break;
 *    	case "Logout";
 ...
 *    	문장1
 *    	break;
 *     case 2: ===> 2번을 찾을 경우 4번까지 갔다가 나온다
 *    	문장2
 *     case 3: ===> 3번을 찾을 경우 4번까지 갔다가 나온다
 *    	문장3
 *     case 4:
 *    	문장4
 *    	break;
 ...
 * 	  ------------- 목록, 페이징 (화면 출력)
 *     1) for : 반복횟수가 지정된 경우
 *     2) while : 반복횟수를 알 수 없는 경우
 *     			=> 파일 읽기, 데이터베이스
 *     3) do ~ while : 한 번 이상을 수행할 경우
 *     
 * 	3. 반복 제어문장
 * 	  ------------- 페이징
 */
...
//		}
		...
		try {
			...
			FileReader fr=
					new FileReader("C:\\javaDev\\javaStudy\\2주차_정리_1\\src\\제어문_1.java");
			int i=0;
			while((i=fr.read())!=-1) {
