...
public class StudentSystem {
	private Student[] std=new Student[3]; // 공간만 만들어줬을뿐 메모리 할당x
	// Student s=new Student();
	//1. 학생 초기화
	/*
	 * 1. 명시적 초기화 => (단점)객체 생성시 모든 객체값이 동일
	 * ---------------------------------------
	 * 2. 생성자 이용
	 * 3. 초기화 블록
	 * 4. 메소드 이용 => 지금 쓰고 있는 방식
	 * ------------------------------ 입력값을 받거나 외부에서 데이터 읽기
	 *                                           --------------- 크롤링, 파일
	 *  ***class 영역에서는 선언과 동시에 초기화
	 *     --------- (제어문, 외부 라이브러리 호출 불가능) => {}에서 가능
	 *                                           
	 */
	public void init() {
		Scanner scan=new Scanner(System.in);
		for(int i=0;i<std.length;i++) {
			std[i]=new Student(); // 배열은 모든 저장공간이 null. 따라서, 메모리 연결해야함
			    // ------------- 메모리 연결하는 부분
	...
	//1-1. 출력
	public void display() // void display()를 작성할 경우 default => com.sist.main4 안에서만 접근이 가능
	                      // com.sist.main5 접근을 위해서는 public 첨가
	{
		for(Student s:std) {
	...
	{
		for(int i=0;i<std.length;i++) {
			std[i].setAvg(std[i].getTotal()/3.0); // 변수(데이터) 비공개
			// sti[i].avg=std[i].getTotal()/3.0; // 변수(데이터) 공개
		}
	}
	//4. 학점 구하기
