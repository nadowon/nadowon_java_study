 ...
 * 267p => try~catch(예외복구)
 * try : 정상실행하는 코딩
 * catch : 예외가 발생 시에 복구하는 과정 (여러개 사용이 가능 => 순서) => 계층구조
 * -----------------------------------------------------------------
 ...
 *            = 스프링에서 주로 사용하는 기술 (리클렉션)
 *                                      ------- private까지 접근이 가능
 *            A a=new A(); Class.forName("A")
 *         => InterruptedException : 쓰레드 (한 개의 프로세스안에서 여러 개의 프로그램을 동시에 사용)
 *            충돌과 관련
 *         => java.lang / java.util => 예외처리와 관련 없다
 *         => 예외처리는 필수 : 정상상태를 유지 => 견고한 프로그램
 *         
 *     2) UnCheckedException : 예외처리를 필요한 경우에만 사용
 *                 Exception
 *                     |
 ...
 *                         int     => Integer
 *                         double  => Double
 *                 Integer.pareInt(" 10"); => 윈도우/Web(문자열만 사용, 정수라는 개념이 없음)
 *                 웹에서 전송된 데이터를 받는 경우
 *                 -------------------------
 ...
 *             
 *             NullPointerExecption : 객체나 배열에 주소가 없는 경우
 *             A a; int[] arr; ==> null
 *             --- A a=new A(); 초기화 시켜줘야 함
 *             
...
public class 예외처리_예외복구_1 {
	...
	public static void main(String[] args) {
		...
		try // for문을 종료하고 catch를 잡아서 도중에 끝남
		{
			for(int i=1;i<=10;i++) {
