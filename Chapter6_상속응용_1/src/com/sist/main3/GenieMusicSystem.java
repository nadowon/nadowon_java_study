...
public class GenieMusicSystem {
	protected Music[] musics=new Music[50]; // static 모든 사람에게 공유
...
//				System.out.println("앨범명:"+album.get(i).text());
				...
				if(s.equals("유지")) {
					...
					state=s.replaceAll("[0-9]", ""); // 한글만 나오게 함 // 상승 하강
					value=s.replaceAll("[가-힣]", ""); // 숫자만 나오게 함 // 1~9
				}
				/*
				 * split
				 * replaceAll
...
//				System.out.println("=============================");
	...
	public Music[] musicFind(String fd,int type) // 메소드 하나를 가지고 두 개의 변수 (구분자)를  // 변수 따로따로 메소드 따로따로 만들수있음
	{
		int i=0;
		for(Music m:musics) {
